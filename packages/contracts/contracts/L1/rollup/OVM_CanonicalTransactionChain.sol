  ...
  Lib_AddressResolver
{
  ...
  uint256 public constant ENQUEUE_GAS_COST = 60000;
  uint256 public ENQUEUE_L2_GAS_PREPAID =
    L2_GAS_DISCOUNT_DIVISOR * ENQUEUE_GAS_COST;
  ...
  function enqueue(
    ...
    if (_gasLimit > ENQUEUE_L2_GAS_PREPAID) {
      uint256 gasToConsume = (_gasLimit - ENQUEUE_L2_GAS_PREPAID) /
        L2_GAS_DISCOUNT_DIVISOR;
    ...
    bytes32 transactionHash = keccak256(
      abi.encode(msg.sender, _target, _gasLimit, _data)
    );
  ...
  function appendSequencerBatch() public override {
    ...
    // later on. Saves gas.
    bytes memory hashMemory = new bytes(
      (msg.data.length / totalElementsToAppend) * 2
    );
  ...
  ) internal view returns (Lib_OVMCodec.QueueElement memory _element) {
    // The underlying queue data structure stores 2 elements
   ...
   * @return Length of the queue.
   */
  function _getQueueLength(iOVM_ChainStorageContainer _queueRef)
    internal
    view
    returns (uint40)
  {
  ...
  function _getSequencerLeafHash(
    ...
    assembly {
      ...
      calldatacopy(
        ...
        add(_nextTransactionPtr, 3),
        _txDataLength
      )

      leafHash := keccak256(
        chainElementStart,
        add(BYTES_TILL_TX_DATA, _txDataLength)
      )
    }
  ...
  function _getSequencerLeafHash(
    ...
    assembly {
      ...
      leafHash := keccak256(
        chainElementStart,
        add(BYTES_TILL_TX_DATA, txDataLength)
      )
    }
  ...
  function _verifySequencerTransaction(
    ...
    require(
      _verifyElement(leafHash, _batchHeader, _inclusionProof),
